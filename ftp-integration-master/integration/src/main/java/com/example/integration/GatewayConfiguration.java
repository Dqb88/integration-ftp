...
class GatewayConfiguration {
	...
	@Bean
	DefaultFtpSessionFactory two(@Value("${ftp2.username}") String username, @Value("${ftp2.password}") String asg, @Value("${ftp2.host}") String host, @Value("${ftp2.port}") int port) {
		return this.createSessionFactory(username, asg, host, port);
	}

	@Bean
	DefaultFtpSessionFactory one(@Value("${ftp1.username}") String username, @Value("${ftp1.password}") String asg, @Value("${ftp1.host}") String host, @Value("${ftp1.port}") int port) {
		return this.createSessionFactory(username, asg, host, port);
	}

	private DefaultFtpSessionFactory createSessionFactory(String username, String asg, String host, int port) {
		var defaultFtpSessionFactory = new DefaultFtpSessionFactory();
		defaultFtpSessionFactory.setPassword(asg);
